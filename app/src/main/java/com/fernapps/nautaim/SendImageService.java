...
/**
 * Created by FeRN@NDeZ on 06/04/2017.
 */
...
public class SendImageService extends Service {
    ...
    public void setPrefValue(String key, String value) {
        Intent prefIntent = new Intent(SendImageService.this, PreferencesHelperService.class);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(SendImageService.this);
        ...
        new SendImageService.SendEmail().execute(intent.getStringExtra("colaid"));
    ...
    private class SendEmail extends AsyncTask<String, String, String> {
        protected String doInBackground(String... path) {
            ...
            Cursor c = db.rawQuery("SELECT * FROM cola WHERE _id = " + path[0], null);
            c.moveToFirst();
            ...
            Cursor cA = db.rawQuery("SELECT * FROM mensajes WHERE _id = " + msgid, null);
            cA.moveToFirst();
            String fileToSendPath = cA.getString(7);
            ...
            String[] result = EmailImageUtil.sendEmail(session, fromEmail, toEmail, asunto, emailBody, SendImageService.this, fileToSendPath, fromName);
        ...
        protected void onPostExecute(String result) {
            stopService(new Intent(SendImageService.this, SendMailHelperService.class));
            if (result != null) {
                if (result.equals("1")) {
                    Cursor c = db.rawQuery("SELECT * FROM cola", null);
                    if (c.getCount() > 0) {
                        sendBroadcast(new Intent("updateConversations"));
                        startService(new Intent(SendImageService.this, SendMailHelperService.class));
                        stopSelf();
                    } else {
                        try {
                    ...
                    String error = result;
                    Notification.Builder mBuilder = new Notification.Builder(SendImageService.this);
                    mBuilder.setSmallIcon(android.R.drawable.ic_dialog_email)
                    ...
                    Intent intent = new Intent(SendImageService.this, SendMailHelperService.class);
                    PendingIntent pendingIntent = PendingIntent.getService(SendImageService.this, 2, intent, PendingIntent.FLAG_UPDATE_CURRENT);
                ...
                NotificationManager notificationManager = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
                Notification.BigTextStyle big = new Notification.BigTextStyle(mBuilder);
                big.bigText("Ha ocurrido un error en la conexión de red y no hemos podido descargar el mensaje.")
                        .setBigContentTitle("Error:");
                notificationManager.notify(78, big.build());
                sendBroadcast(new Intent("updateConversations"));
                stopForeground(true);
